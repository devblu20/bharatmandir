...
function UserMarker({ position, label }) {
  const map = useMap();
  ...
  return position ? (
    <Marker position={position} icon={ICONS.user}>
      <Popup><div style={{ padding: 8, fontFamily: 'var(--font-display)', fontSize: 13, color: '#2563EB', fontWeight: 700 }}>📍 {label}</div></Popup>
    </Marker>
  ) : null;
}
...
export default function MapPage() {
  ...
  // ── Translation hooks ──
  const { translated: displayTemples } = useTranslatedTemples(temples);
  const { translated: displayNearby  } = useTranslatedTemples(nearby);
  ...
  const handleNearMe = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setGpsLoading(false);
        alert(err.code === 1 ? t('map.geo_denied') : t('map.geo_error'));
      },
  ...
  const getIcon = (temple) => { if (temple.is_jyotirlinga) return ICONS.jyotirlinga; if (temple.is_shaktipeeth) return ICONS.shaktipeeth; return ICONS.normal; };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div className="map-topbar">
        ...
        <button className="gps-btn" onClick={() => setTileLayer(l => l === 'streets' ? 'satellite' : 'streets')}>
          <Layers size={14} />
          {tileLayer === 'streets' ? t('map.satellite_view') : t('map.street_view')}
        </button>
        <span style={{ marginLeft: 'auto', fontSize: 13, color: 'var(--text-light)' }}>
          {loading ? t('loading') : `${temples.length} ${t('map.temples_on_map', { count: '' }).trim()}`}
      ...
      <div className="map-container">
        <MapContainer center={[22.5, 80.0]} zoom={5} style={{ height: '100%', width: '100%' }} zoomControl>
          ...
          {userPos && <UserMarker position={userPos} label={t('map.you_are_here')} />}
            ...
            <div className="nearby-card">
              ...
              {!nearbyLoad && nearby.length > 0 && (
                <div className="nearby-list">
                  {displayNearby.map((temple) => (
                    <Link key={temple.id} to={`/temple/${temple.slug}`} className="nearby-item">
                      <div className="nearby-icon">🛕</div>
                      <div className="nearby-info">
                        <div className="nearby-name">{temple.name}</div>
                        <div className="nearby-dist">📍 {temple.distance_km} km away · {temple.city}</div>
                      </div>
                    </Link>
                  ))}
                </div>
